...
public class BlockCrystbud extends BlockBush
{
    ...
    public boolean canPlaceBlockAt(World worldIn, BlockPos pos)
    {
        ...
        return downState.getBlock() == Blocks.NETHERRACK && super.canPlaceBlockAt(worldIn, pos);
    }

    /**
     * Can only sustain on netherrack
     */
    @Override
    ...
    /**
     * Generate redstone particles with medium purple color around the herb
     * RGB: 128, 0, 128 (medium purple)
     * Particles float around the top of the herb to create a crystallization effect
     */
    @Override
    @SideOnly(Side.CLIENT)
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand)
    {
        ...
        {
            // REDSTONE particle with color #6439b5 (RGB: 100, 57, 181)
            // In Minecraft 1.12.2, REDSTONE particle color is specified using velocity parameters (dx, dy, dz)
            // Color values are normalized (0.0-1.0) where 1.0 = 255
            worldIn.spawnParticle(
                EnumParticleTypes.REDSTONE,
                x + offsetX,
                y,
                z + offsetZ,
                100.0D / 255.0D,  // Red component (100/255 = 0.392)
                57.0D / 255.0D,   // Green component (57/255 = 0.224)
                181.0D / 255.0D,  // Blue component (181/255 = 0.710)
                new int[0]
            );
        }
    }
