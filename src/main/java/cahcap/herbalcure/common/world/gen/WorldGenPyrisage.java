...
/**
 * World generator for Pyrisage in the Nether
 * Generates small patches (2-3 herbs) on soul sand
 */
public class WorldGenPyrisage implements net.minecraftforge.fml.common.IWorldGenerator
{
    private WorldGenerator herbGenerator;

    public WorldGenPyrisage()
    {
        this.herbGenerator = new WorldGenPyrisagePatch();
    }
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider)
    {
        ...
        {
            int attempts = 2; // 2 attempts per chunk to find suitable soul sand
            for (int attempt = 0; attempt < attempts; attempt++)
            {
                int xPos = x + random.nextInt(16);
                int zPos = z + random.nextInt(16);
                int yPos = findSoulSandLevel(world, xPos, zPos);
                if (yPos > 0)
                {
    ...
    private int findSoulSandLevel(World world, int x, int z)
    {
        // In the Nether, search from top to bottom for soul sand
    ...
    private boolean isSuitablePosition(World world, BlockPos pos)
    {
        ...
        if (groundBlock != Blocks.SOUL_SAND)
        {
            return false;
        }
    ...
    /**
     * WorldGenerator for generating a patch of 2-3 Pyrisage herbs
     */
    private static class WorldGenPyrisagePatch extends WorldGenerator
    {
        @Override
        public boolean generate(World worldIn, Random rand, BlockPos position)
        {
            ...
            for (int i = 0; i < 10 && placedCount < herbCount; i++)
            {
                ...
                // Find soul sand level at this position
                int yPos = findSoulSandLevelAt(worldIn, herbPos);
                if (yPos > 0)
                {
                    BlockPos finalPos = new BlockPos(herbPos.getX(), yPos, herbPos.getZ());
                    if (canPlaceHerb(worldIn, finalPos))
                    {
                        worldIn.setBlockState(finalPos, ModRegistries.blockPyrisage.getDefaultState(), 2);
                        placedCount++;
                    }
                }
            }
        ...
        private int findSoulSandLevelAt(World world, BlockPos pos)
        {
